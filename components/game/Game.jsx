...
const Game = () => {
  ...
  const [words, setWords] = useState([]); // 단어
  ...
  useEffect(() => {
    ...
    // 단어 받기
    const handleWords = (wordsArr) => setWords(wordsArr);
    socket.on("words", handleWords);

    return () => socket.off("words", handleWords);
  }, [socket]);
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[700px] w-full font-pretendard">
        <div className="w-full max-w-2xl min-w-[320px] bg-white rounded-3xl shadow-2xl px-10 py-16 flex flex-col items-center border-b-4 border-[#2482c5]">
          <img src="/wooriduri-icon.png" className="h-16 mb-4" alt="wooriduri icon" />
          <div className="text-[#2482c5] text-4xl font-extrabold mb-4 tracking-wider">영어 단어 빨리 치기</div>
          <div className="mb-10 text-2xl text-gray-600 text-center font-bold">게임 정보를 불러오는 중...</div>
        </div>
      </div>
    );
  } else if (countdown !== null && !showGame) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[700px] w-full font-pretendard">
        <div
        ...
        >
          ...
          <div className="mb-2 text-4xl font-black text-[#1e88e5] drop-shadow tracking-wider flex gap-2 items-center">
            <span className="animate-pulse">{countdown > 0 ? countdown : "시작!"}</span>
            <span className="text-xl font-normal text-gray-500">{countdown > 0 ? "초 후 시작" : ""}</span>
          </div>
          <div className="mt-8 w-full flex justify-center">
            <div className="h-2 w-64 bg-gray-200 rounded-full overflow-hidden">
              <div className="h-2 rounded-full bg-gradient-to-r from-[#42a5f5] to-[#2482c5] transition-all duration-500" style={{ width: `${(countdown / 3) * 100}%` }} />
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-col items-center justify-center min-h-[700px] w-full font-pretendard">
        ...
        >
          <div className="flex items-center gap-3 mb-6">
            <img src="/wooriduri-icon.png" className="h-10" alt="wooriduri icon" />
            <div className="text-[#2482c5] text-3xl md:text-4xl font-extrabold tracking-wider">영어 단어 빨리 치기</div>
          </div>
          <div className="mb-8 text-2xl text-gray-500 font-bold text-center">
            <span className="text-[#2482c5]">제시된 단어</span>를 <span className="text-[#1e88e5]">빠르게</span> 입력하세요!
          </div>
          <div className="w-full flex items-center justify-center mb-8 h-24 md:h-28" style={{ userSelect: "none" }}>
            <span className="text-5xl md:text-6xl font-black text-[#1e88e5] tracking-wide drop-shadow">{currentWord < words.length ? words[currentWord] : "🎉 게임 끝! 🎉"}</span>
          </div>
          {currentWord < words.length && (
            <input
              ...
              className={
                ...
                (inputError ? "border-red-500 bg-red-100 animate-shake" : "border-[#90caf9] focus:border-[#2482c5]")
              }
          ...
          <div className="w-full flex items-center justify-between text-2xl md:text-3xl mt-8">
            <span className="text-gray-600">
              남은 단어: <span className="font-bold">{words.length - currentWord}</span>
            </span>
            <span className="text-[#2482c5] font-bold">{finishTime ? `기록: ${(finishTime / 1000).toFixed(2)}초` : `진행: ${currentWord} / ${words.length}`}</span>
          </div>
          {currentWord >= words.length && (
            <button
              onClick={() => {
